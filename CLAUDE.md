...
- **stdout is reserved for the MCP protocol.** Never `print()` or log to stdout. All logging goes to a file under `LOG_DIR` (default `<WORKSPACE_DIR>/logs`, git-ignored). `_activate_log_file()` builds the path **per search** as `<LOG_DIR>/<index-prefix>/<sanitized-index>-<YYYY-MM-DD_HH-MM-SS>.log`, creating the folder if missing; `_index_prefix()` derives that folder by dropping the trailing date/rollover/wildcard part of the index pattern (`artifactory-idcevo-*` and `artifactory-idcevo-2026.08.10` → `artifactory-idcevo`, `ci-*` → `ci`, `*` → `all-indices`). Until the first search runs, records are held in a `MemoryHandler` and flushed into that first file (so startup/time/index messages aren't lost). Each subsequent search opens its own file. Adding stdout output will corrupt the protocol stream. The `elasticsearch`/`elastic_transport`/`urllib3` loggers are pinned to `WARNING` so per-request HTTP lines don't flood the log.
...
There is **no test suite, linter config, or build step** in this repo. The server is exercised through VS Code: open `.vscode/mcp.json` → **Start**, or Command Palette → `MCP: List Servers`. View server logs via **Show Output** there, and tool activity in the per-search `logs/<index-prefix>/<index>-<timestamp>.log` files.
